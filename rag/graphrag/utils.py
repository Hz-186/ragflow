...
def get_from_to(node1, node2):
    ...
    推演：("北京大学", "张三") → ("张三", "北京大学")   # "张" 的编码比 "北" 小？
    实际按 Python 字符串比较规则，谁小谁在前，规则唯一就行。
    """
    if node1 < node2:
        return (node1, node2)
    else:
...
def merge_tuples(list1, list2):
    ...
    N 跳邻居枚举的核心引擎。推演：
        ...
        list2 = [("北京大学", "李四"), ("北京大学", "王五"), ("张三", "北京大学")]
                # 北京大学 出发的所有边（最后一条是刚走过来的那条）

        对路径 ("张三", "北京大学")：
            尾巴是 "北京大学" → 挑 list2 里以 "北京大学" 开头的边：
            ("北京大学", "李四")：没在这条路径里相邻走过 → 接上 → ("张三","北京大学","李四")
            ("北京大学", "王五")：同理 → ("张三","北京大学","王五")
            ("张三", "北京大学") 反向 ("北京大学","张三") 相邻走过 → 跳过（防回头路）
